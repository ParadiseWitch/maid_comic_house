...
class CopymangaSpider(Spider):
    ...
    def spider_comic(self, comic: Comic, range_fn: Callable[[List], List] = lambda arr: arr[0:sys.maxsize]):
        ...
        爬全部漫画
        """
        mlogger.info('开始下载漫画,url={}'.format(comic.url))
        ...
        mlogger.info('漫画名:{}'.format(comic.name))
        ...
        target_chapter_list = []
        for chapter_name_and_url in target_chapter_name_and_url_list:
            chapter_url = chapter_name_and_url['url']
            chapter_name = chapter_name_and_url['name']
            chapter = Chapter(comic.id, chapter_url, chapter_name)
            chapter.imgs = []
    ...
    def spider_chapter_by_url(self, url: str):
        ...
        if comic is None:
            mlogger.warning('查询到comic_id={}的漫画不存在,现在开始爬取漫画信息记录到db'.format(comic_id))
            self.spider_base_comic_info_by_url(comic_id)
